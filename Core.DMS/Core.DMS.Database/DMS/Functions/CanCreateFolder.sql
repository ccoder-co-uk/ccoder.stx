...
CREATE FUNCTION [DMS].[CanCreateFolder]
(
...
AS
BEGIN
	DECLARE @FolderId as uniqueidentifier = (SELECT TOP 1 [Id] FROM [DMS].[Folders] f WHERE f.[Path] = @Path AND f.AppId=@AppId)
	...
	DECLARE @CanCreate as tinyint = (
		SELECT TOP 1 CASE 
			WHEN EXISTS (
				...
				WHERE (r.[Privs] LIKE '%folder_create%' OR r.[Privs] LIKE '%app_admin%') AND r.AppId=@AppId
			) OR EXISTS (				
				SELECT * FROM [Security].[Roles] r
